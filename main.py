...
BEAM_STRIPE_A = (0.95, 0.75, 0.10, 1)
BEAM_STRIPE_B = (0.12, 0.12, 0.13, 1)
...
BIN_ASPECT = 389.0 / 420.0
CONE_ASPECT = 386.0 / 420.0
BARRIER_ASPECT = 410.0 / 420.0

TREE_ASPECT = 470.0 / 520.0
BENCH_ASPECT = 320.0 / 320.0
LAMP_ASPECT = 497.0 / 560.0
BUILDING_TEXTURES = [
    ("env_building_vinyl.png", 583.0 / 640.0),
    ("env_building_ramen.png", 822.0 / 640.0),
    ("env_building_tower.png", 637.0 / 640.0),
]
ROADSIDE_ASSETS = [
    ("env_tree.png", TREE_ASPECT, 0.30),
    ("env_bench.png", BENCH_ASPECT, 0.19),
    ("env_lamp.png", LAMP_ASPECT, 0.34),
]
ROADSIDE_MIN_GAP = 1.1
ROADSIDE_MAX_GAP = 2.0
...
# =================================================================
# ONEMLI PERFORMANS NOTU:
# Acilis animasyonunu 29 AYRI PNG dosyasi olarak oynatmak telefonda
# gozle gorulur bir yavaslamaya (her karede diskten okuma/decode
# gecikmesi) yol aciyordu - "yarim acilip sonra birden tamamlanma"
# hissi buradan kaynaklaniyordu. Cozum: butun kareler TEK BIR
# "sprite sheet" (splash_sheet.png) icinde birlestirildi, sadece bir
# kez yukleniyor, kareler arasi gecis GPU uzerinde aninda oluyor.
SPLASH_SHEET_COLS = 6
SPLASH_SHEET_ROWS = 5
SPLASH_SHEET_CELL = 170
SPLASH_TOTAL_FRAMES = 29
...
def get_texture(filename):
    if filename not in _texture_cache:
        try:
            _texture_cache[filename] = CoreImage(filename).texture
        except Exception:
            _texture_cache[filename] = None
    return _texture_cache[filename]


class SplashScreen(Widget):
    _sheet_texture = None  # tum widget orneklerinde paylasilan, tek seferlik yuklenir
    ...
    def __init__(self, on_finish, **kwargs):
        ...
        if SplashScreen._sheet_texture is None:
            SplashScreen._sheet_texture = CoreImage("splash_sheet.png").texture

        self.logo_img = Image(
            allow_stretch=True, keep_ratio=True, size_hint=(None, None),
        )
        self.logo_img.texture = self._get_frame_texture(0)
    ...
    def _get_frame_texture(self, index):
        col = index % SPLASH_SHEET_COLS
        row = index // SPLASH_SHEET_COLS
        cell = SPLASH_SHEET_CELL
        x_px = col * cell
        y_px = (SPLASH_SHEET_ROWS * cell) - (row * cell) - cell
        return self._sheet_texture.get_region(x_px, y_px, cell, cell)
    ...
    def _advance(self, dt):
        if self.finished:
            return False
        if self.frame_idx < SPLASH_TOTAL_FRAMES - 1:
            self.frame_idx += 1
            self.logo_img.texture = self._get_frame_texture(self.frame_idx)
            return True
        self._timer += dt
...
class GameWidget(Widget):
    ...
    def _layout_metrics(self):
        self.char_h = self.height * 0.15
        self.char_w = self.char_h * FRAME_ASPECT
        ...
        while x < self.width * 1.4:
            ...
            texture_name = None
            if bcount % 3 == 1:
                tex_choice = BUILDING_TEXTURES[bcount % len(BUILDING_TEXTURES)]
                texture_name, tex_aspect = tex_choice
                h = self.height * rnd.uniform(0.16, 0.24)
                w = h * tex_aspect
            self.buildings.append({
                "x": x, "w": w, "h": h, "windows": windows, "texture": texture_name,
            })
    ...
    def _update_playing(self, dt):
        ...
        if not self.jumping and not self.sliding:
            lean_threshold = LANE_LEAN_MAX_DEG * 0.45
            if self.lane_lean > lean_threshold:
                if self.current_frame_idx != "lean_right":
                    self.current_frame_idx = "lean_right"
                    self.char_img.source = "run_lean_right.png"
            elif self.lane_lean < -lean_threshold:
                if self.current_frame_idx != "lean_left":
                    self.current_frame_idx = "lean_left"
                    self.char_img.source = "run_lean_left.png"
            else:
        ...
        self._spawn_roadside(dt)
        self._move_and_check(dt)
        self._move_roadside(dt)

    def _spawn_roadside(self, dt):
        self.roadside_timer -= dt
        if self.roadside_timer <= 0:
            self.roadside_timer = random.uniform(ROADSIDE_MIN_GAP, ROADSIDE_MAX_GAP)
            side = random.choice(["left", "right"])
            asset_idx = random.randrange(len(ROADSIDE_ASSETS))
            self.roadside_props.append({
                "side": side, "asset": asset_idx, "t": 1.0, "y": self.horizon_y,
            })

    def _move_roadside(self, dt):
        span = max(1.0, self.horizon_y - self.player_y)
        t_rate = (self.speed / span) * dt
        still = []
        for p in self.roadside_props:
            p["t"] -= t_rate
            p["y"] = self.perspective_y(p["t"])
            if p["t"] >= -0.1:
                still.append(p)
        self.roadside_props = still
    ...
    def _draw_buildings(self):
        ...
        for b in self.buildings:
            ...
            if bx < self.width * 1.3:
                tex = get_texture(b["texture"]) if b["texture"] else None
                if tex:
                    Color(1, 1, 1, 1)
                    Rectangle(texture=tex, pos=(bx, self.horizon_y), size=(b["w"], b["h"]))
                else:
    ...
    def _draw_roadside_props(self):
        for p in self.roadside_props:
            asset_name, aspect, size_ratio = ROADSIDE_ASSETS[p["asset"]]
            tex = get_texture(asset_name)
            if not tex:
                continue
            scale = self.distance_scale(p["y"])
            h = self.char_h * (size_ratio / 0.22) * scale
            w = h * aspect
            x = self.sidewalk_prop_x(p["side"], p["y"])
            Color(0, 0, 0, 0.25 * scale)
            Ellipse(pos=(x - w * 0.35, p["y"] - h * 0.06), size=(w * 0.7, w * 0.22))
            Color(1, 1, 1, 1)
            Rectangle(texture=tex, pos=(x - w / 2.0, p["y"]), size=(w, h))

    def sidewalk_prop_x(self, side, y):
        sw_l, sw_r = self.sidewalk_bounds(y)
        rd_l, rd_r = self.road_bounds(y)
        if side == "left":
            return (sw_l + rd_l) / 2.0
        return (sw_r + rd_r) / 2.0

    def _draw_sidewalk_and_road(self):
        sw_bl, sw_br = self.sidewalk_bounds(0)
        sw_tl, sw_tr = self.sidewalk_bounds(self.horizon_y)
        sw_tex = get_texture("texture_sidewalk.png")
        if sw_tex:
            Color(1, 1, 1, 1)
        else:
        ...
        Mesh(
            ...
            indices=[0, 1, 2, 3], mode="triangle_fan", texture=sw_tex,
        )

        road_tex = get_texture("texture_road.png")
        if road_tex:
            Color(1, 1, 1, 1)
        else:
            Color(*ROAD_COLOR)
        Mesh(
            ...
            indices=[0, 1, 2, 3], mode="triangle_fan", texture=road_tex,
        )
    ...
    def _draw_obstacles(self):
        for ob in self.obstacles:
            ...
            if ob["type"] == "bin":
                bin_tex = get_texture("obstacle_bin.png")
                bh = self.char_h * 0.62 * scale
                bw = bh * BIN_ASPECT
                self._draw_ground_shadow(lx, ob["y"], bw * 0.85)
                if bin_tex:
                    Color(1, 1, 1, 1)
                    Rectangle(texture=bin_tex, pos=(lx - bw / 2.0, ob["y"] - bh * 0.10), size=(bw, bh))
                else:
                    Color(*BIN_BODY_COLOR)
                    Rectangle(pos=(lx - bw / 2.0, ob["y"] - bh * 0.10), size=(bw, bh))

            elif ob["type"] == "barrier":
                bar_tex = get_texture("obstacle_barrier.png")
                bh = self.char_h * 0.48 * scale
                bw = bh * BARRIER_ASPECT
                self._draw_ground_shadow(lx, ob["y"], bw * 0.95)
                if bar_tex:
                    Color(1, 1, 1, 1)
                    Rectangle(texture=bar_tex, pos=(lx - bw / 2.0, ob["y"] - bh * 0.12), size=(bw, bh))
                else:
                    Color(*BARRIER_COLOR_A)
                    Rectangle(pos=(lx - bw / 2.0, ob["y"] - bh * 0.12), size=(bw, bh))

            elif ob["type"] == "cone":
                cone_tex = get_texture("obstacle_cone.png")
                ch = self.char_h * 0.42 * scale
                cw = ch * CONE_ASPECT
                self._draw_ground_shadow(lx, ob["y"], cw * 0.9)
                if cone_tex:
                    Color(1, 1, 1, 1)
                    Rectangle(texture=cone_tex, pos=(lx - cw / 2.0, ob["y"] - ch * 0.12), size=(cw, ch))
                else:
                    Color(*CONE_COLOR)
                    Triangle(points=[lx, ob["y"] + ch, lx - cw / 2.0, ob["y"], lx + cw / 2.0, ob["y"]])

            else:  # beam - havadan gecen tehlike bariyeri (sari-siyah serit)
                w = lane_w * 0.95 * scale
                h = self.char_h * 0.22 * scale
                by = ob["y"] + self.char_h * 0.30 * scale
                bx = lx - w / 2.0
                seg = w / 7.0
                for i in range(7):
                    Color(*(BEAM_STRIPE_A if i % 2 == 0 else BEAM_STRIPE_B))
                    Rectangle(pos=(bx + i * seg, by), size=(seg, h))
                Color(0, 0, 0, 0.35)
                Line(rectangle=(bx, by, w, h), width=1.2)
